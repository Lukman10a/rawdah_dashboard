  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
        ...
        />
        <KpiCard
          label="Hourly Disbursement"
          value={`$${hourlyTotal.toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
          hint={`${hourly.length} teachers`}
        />
        <KpiCard
          label="Salaried Disbursement"
          value={`$${salariedTotal.toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
          hint={`${salaried.length} teachers`}
        />
        <KpiCard
          label="Forecast Next Month"
          value={`$${(total * 1.028).toLocaleString(undefined, { maximumFractionDigits: 0 })}`}
          hint="AI · +2.8%"
          accent="gold"
        />
      </div>
        ...
        <Card className="lg:col-span-2 overflow-hidden">
          <div className="p-6 border-b border-hairline">
            <h3 className="font-display italic text-lg">
              Hourly Payroll Detail
            </h3>
            <p className="text-xs text-ink-muted">
              Pay = hourly rate × hours logged
            </p>
          </div>
            ...
            <tfoot>
              <tr className="bg-cream/60 border-t border-hairline text-navy font-medium">
                ...
                <td className="px-6 py-3 text-right font-mono">
                  $
                  {hourlyTotal.toLocaleString(undefined, {
                    minimumFractionDigits: 2,
                  })}
                </td>
              </tr>
        ...
        <div className="space-y-6">
          ...
          <Card className="p-5 bg-navy text-cream border-gold/30">
            ...
            <p className="text-sm mt-2 leading-relaxed">
              Ramadan schedule will shift ~24 daytime hours to evening
              tutorials. Estimated payroll delta:{" "}
              <span className="text-gold font-medium">+$2,412</span>. Salaried
              staff unchanged.
            </p>

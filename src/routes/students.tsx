...
function StudentsPage() {
  const avgPerf = Math.round(students.reduce((s, x) => s + x.performance, 0) / students.length);
  const avgAtt = Math.round(students.reduce((s, x) => s + x.attendance, 0) / students.length);
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
        <KpiCard label="Enrolled" value="1,248" hint="+58 this term" accent="green" />
        <KpiCard label="Avg Performance" value={`${avgPerf}%`} progress={avgPerf} />
        <KpiCard label="Avg Attendance" value={`${avgAtt}%`} accent="muted" hint="last 30 days" />
        <KpiCard label="At-risk" value="14" hint="AI flagged this month" accent="gold" />
      </div>
          ...
          <tbody className="text-sm divide-y divide-hairline">
            {students.map((s) => (
              <tr key={s.id} className="hover:bg-cream/40">
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-ink-muted font-mono">{s.id}</div>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1 bg-navy/5 rounded-full overflow-hidden">
                      <div className="h-full bg-navy" style={{ width: `${s.performance}%` }} />
                    </div>
                ...
                <td className="px-6 py-4 text-ink-muted text-xs">{s.lastReport}</td>
              </tr>
            ))}
          </tbody>
        </table>

...
import { teachers, computePay } from "../lib/mock-data";
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-6">
        <KpiCard label="Total Teachers" value={String(teachers.length)} hint={`${hourly.length} hourly · ${salaried.length} salaried`} />
        <KpiCard label="Hours Logged" value={totalHours.toFixed(1)} hint="this month" />
        <KpiCard label="Avg Rating" value={`${avgRating}/5`} accent="gold" hint="based on peer & parent feedback" />
      ...
      <Card className="overflow-hidden">
        <div className="p-6 border-b border-hairline flex justify-between items-center">
          ...
          <div className="flex gap-1.5">
            <span className="text-[10px] px-2 py-1 rounded-full bg-cream border border-hairline">All</span>
            <span className="text-[10px] px-2 py-1 rounded-full text-ink-muted">Hourly</span>
            <span className="text-[10px] px-2 py-1 rounded-full text-ink-muted">Salaried</span>
          </div>
        </div>
                ...
                <tr key={t.id} className="hover:bg-cream/40 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                      t.employmentType === "Hourly" ? "bg-gold/15 text-navy" : "bg-navy/5 text-navy"
                    }`}>
                      {t.employmentType}
                    </span>
                  </td>
                  <td className="px-6 py-4 font-mono text-xs">
                    {t.employmentType === "Hourly" ? `$${t.hourlyRate.toFixed(2)}/hr` : `$${t.salaryMonthly?.toLocaleString()}/mo`}
                  </td>
                  <td className="px-6 py-4 font-mono text-xs">{t.employmentType === "Hourly" ? t.hoursLogged.toFixed(1) : "—"}</td>
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1 bg-navy/5 rounded-full overflow-hidden">
                        <div className="h-full bg-gold" style={{ width: `${t.attendance}%` }} />
                      </div>
                  ...
                  <td className="px-6 py-4">
                    <div className="inline-flex items-center gap-1 text-xs">
                      <Star className="size-3 fill-gold text-gold" /> {t.rating.toFixed(1)}
                    </div>
                  </td>
